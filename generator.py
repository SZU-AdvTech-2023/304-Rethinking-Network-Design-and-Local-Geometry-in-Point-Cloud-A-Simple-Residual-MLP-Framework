...
class DentalModelGenerator(Dataset):
    def __init__(self, data_dir=None, split_with_txt_path=None, aug_obj_str=None):
        ...
        if split_with_txt_path:
            ...
            for i in range(len(self.mesh_paths)):
                ...
                if p_id in self.split_base_name_ls:     # 添加在split.txt中的mesh
    ...
    def __getitem__(self, idx):
        ...
        output = {}
        low_feat = mesh_arr.copy()[:,:6].astype("float32")          # 前三个是坐标，后三个是法向量
        #print("low_feat", low_feat.shape)
        seg_label = mesh_arr.copy()[:,6:].astype("int")
        seg_label -= 1 # -1 means gingiva, 0 means first incisor...
        # # [0,....,7] => [11,....,18] or [31,....,38] , [8,....,15]  => [21,....,28] or [41,....,48]
        if self.aug_obj:        # 进行数据增强
            self.aug_obj.reload_vals()      # 获取随机的数据增强值
            """
            ...
            """
            low_feat = self.aug_obj.run(low_feat)       # 应用随机的数组增强值
        low_feat = torch.from_numpy(low_feat)
        low_feat = low_feat.permute(1,0)        # n*3 -> 3*n
        output["feat"] = low_feat
        seg_label = torch.from_numpy(seg_label)
        seg_label = seg_label.permute(1,0)      # n*1 -> 1*n
