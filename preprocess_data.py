...
for i in range(len(stl_path_ls)):
    ...
    if loaded_json['jaw'] == 'lower':
        labels -= 20
    labels[labels//10==1] %= 10         # [1,....,8] => [11,....,18] or [31,....,38] , [9,....,16]  => [21,....,28] or [41,....,48]
    labels[labels//10==2] = (labels[labels//10==2]%10) + 8
    labels[labels<0] = 0            # 牙龈全是0
    ...
    # print(labeled_vertices[6])

    name_id = str(base_name)
    if labeled_vertices.shape[0]>24000:
        labeled_vertices = gu.resample_pcd([labeled_vertices], 24000, "fps")[0]     # 最远点采样24000个点

    #np.save(os.path.join(SAVE_PATH, f"{name_id}_{loaded_json['jaw']}_sampled_points"), labeled_vertices)
    np.save(os.path.join(SAVE_PATH, f"{name_id}_sampled_points"), labeled_vertices)
