...
class Rotation:
    ...
    def augment(self, vert_arr):
        if self.angle_axis == "pca":
            ...
            rotation_mat = gu.axis_rotation(self.angle_axis_val, self.rot_val)      # 无论旋转轴为哪一个，都能获取旋转该轴对应的旋转3*3矩阵.
        ...
        if vert_arr.shape[1]==6:            # 法向量也要跟着旋转
            vert_arr[:,3:] = (rotation_mat @ vert_arr[:,3:].T).T
        return vert_arr
    ...
    def reload_val(self):
        if self.angle_axis == "rand":
            ...
            rand_axis = np.random.permutation(3)[0]         # 0 or 1 or 2
            three_axis = np.array([[1,0,0],[0,1,0],[0,0,1]])
            self.angle_axis_val = three_axis[rand_axis]
