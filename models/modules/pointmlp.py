...
class PointMlpFirstModule(torch.nn.Module):
    def __init__(self, config):
        self.config = config

        super().__init__()
        self.first_sem_model = get_model(num_classes=17,points=24000,num_channels=6)
    ...
    def forward(self, inputs, test=False):
        ...
        cls_pred = self.first_sem_model(inputs[0])
        outputs = {
            "cls_pred": cls_pred
        }
        return outputs