...
def get_nearest_neighbor_idx(org_xyz, sampled_clusters, crop_num=4096):
    ...
    for batch_idx in range(org_xyz.shape[0]):
        #for cluster_num in range(sampled_clusters.shape[1]):
        tree = KDTree(org_xyz[batch_idx, :, :], leaf_size=2)
            # tree = KDTree(np.expand_dims(org_xyz[batch_idx, :, :],axis=0), leaf_size=2)
        #print("sampled_clusters[batch_idx][cluster_num].shape: ", sampled_clusters[batch_idx][cluster_num].shape)
        #indexs = tree.query(sampled_clusters[batch_idx][cluster_num].reshape(-1,3), k=crop_num, return_distance=False)
        indexs = tree.query(sampled_clusters[batch_idx], k=crop_num, return_distance=False)     # indexs [cluster_num,crop_num]
...
def seg_label_to_cent(gt_coords, gt_seg_label):
    # 这里注意，整个batch的所有点云的所有点都会被拿去计算中心，而不是一个点云计算一个中心，如果batchsize大于1的话
    gt_coords = gt_coords.permute(0,2,1)    # [B,3,N] => [B,N,3]
    #gt_coords = gt_coords.view(-1,3)        # [B*N,3]
    #print("gt_coords.shape: ", gt_coords.shape)   # B, N, 3
    gt_coords = gt_coords.reshape(-1, 3)  # [B*N,3]
    gt_seg_label = gt_seg_label.view(-1)    # [B*N*1]

    gt_cent_coords = []
    gt_cent_exists = []
    for class_idx in range(0, 16):      # 牙龈是-1，不会被记录
        cls_cond = gt_seg_label==class_idx      # 取出label为class_idx的点
        
        cls_sample_xyz = gt_coords[cls_cond, :]     # 取出label为class_idx的所有点
        if cls_sample_xyz.shape[0]==0:              # 没有label为class_idx的点， 比如7和15，第三磨牙
            gt_cent_coords.append(torch.from_numpy(np.array([-10,-10,-10])))        # why?
            gt_cent_exists.append(torch.zeros(1))
        else:
            centroid = torch.mean(cls_sample_xyz, axis=0)       # 减少一个维度
            gt_cent_coords.append(centroid)
            gt_cent_exists.append(torch.ones(1))

    gt_cent_coords = torch.stack(gt_cent_coords)        # [16,3]
    gt_cent_coords = gt_cent_coords.view(1, *gt_cent_coords.shape) # 增加0维度并且为1  [1,16,3]
    gt_cent_coords = gt_cent_coords.permute(0,2,1)      # [1,3, 16]
    gt_cent_exists = torch.stack(gt_cent_exists)        # [16,1]
    gt_cent_exists = gt_cent_exists.view(1, -1)   # [1,16]   ele is 0 or 1

    return gt_cent_coords, gt_cent_exists


def seg_label_to_cent_1(gt_coords, gt_seg_label):
    # input: gt_coords: B, 3, N; gt_seg_label: B, 1,N
    gt_coords = gt_coords.permute(0, 2, 1)  # [B,3,N] => [B,N,3]
    # gt_coords = gt_coords.view(-1,3)        # [B*N,3]
    #print("gt_coords.shape: ", gt_coords.shape)  # B, N, 3
    B = gt_coords.shape[0]
    #gt_coords = gt_coords.reshape(B, -1)  # [B,3*N]
    gt_seg_label = gt_seg_label.permute(0, 2, 1).reshape(B,-1)  # [B,N,1]

    gt_cent_coords = []
    gt_cent_exists = []
    for i in range(B):
        for class_idx in range(0, 16):  # 牙龈是-1，不会被记录
            cls_cond = gt_seg_label[i] == class_idx  # 取出label为class_idx的点

            cls_sample_xyz = gt_coords[i][cls_cond, :]  # 取出label为class_idx的所有点
            if cls_sample_xyz.shape[0] == 0:  # 没有label为class_idx的点， 比如7和15，第三磨牙
                gt_cent_coords.append(torch.from_numpy(np.array([-10, -10, -10])))  # why?
                gt_cent_exists.append(torch.zeros(1))
            else:
                ...
                #centroid.reshape(B,3)
                gt_cent_coords.append(centroid)
                gt_cent_exists.append(torch.ones(1))

    gt_cent_coords = torch.stack(gt_cent_coords)  # [B*16,3]
    gt_cent_coords = gt_cent_coords.view(B, 16, 3)
    #gt_cent_coords = gt_cent_coords.view(1, *gt_cent_coords.shape)  # 增加0维度并且为1  [1,16,3]
    gt_cent_coords = gt_cent_coords.permute(0, 2, 1)  # [B,3, 16]
    gt_cent_exists = torch.stack(gt_cent_exists)  # [B*16,1]
    gt_cent_exists = gt_cent_exists.view(B, 16, 1)  # entry is 0 or 1
    gt_cent_exists = gt_cent_exists.permute(0, 2, 1)  # [B,1,16]
...
def get_indexed_features(features, cropped_indexes):
    ...
    Output:
        cropped_item_ls => type torch cuda/np => new batch B, channel, 4096     新的batchsize变为原来的cluster_num倍
    """
    cropped_item_ls = []
    for b_idx in range(len(cropped_indexes)):
        for cluster_idx in range(len(cropped_indexes[b_idx])):
            ...
            cropped_point = features[b_idx][:, cropped_indexes[b_idx][cluster_idx]]     # cropped_indexes[b_idx][cluster_idx]获取的是对应的4096个点的index
